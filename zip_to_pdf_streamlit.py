...
# Helper function to write images to PDF incrementally
def write_images_to_pdf(image_files, output_pdf_path):
    pdf = FPDF(unit="pt", format=[595, 842])  # A4 size
    for image_path in image_files:
        img = process_and_resize_image(image_path)
        temp_image_path = "temp_image.jpg"
        img.save(temp_image_path, "JPEG", quality=85)
        pdf.add_page()
        pdf.image(temp_image_path, 0, 0, 595, 842)  # Full-page image
        os.remove(temp_image_path)
    pdf.output(output_pdf_path)

# Streamlit app
def main():
    st.title("High-Capacity ZIP to PDF Converter")
    st.write("Upload ZIP files containing images. We'll process up to 5000 images efficiently.")
    ...
    if uploaded_files:
        ...
        try:
            ...
            # Extract and sort images from uploaded ZIP files
            ...
            # Split images into batches of 500 for processing
            batch_size = 500
            num_batches = (len(all_image_files) + batch_size - 1) // batch_size
            output_pdf_path = "output.pdf"

            for i in range(num_batches):
                batch_files = all_image_files[i * batch_size:(i + 1) * batch_size]
                st.write(f"Processing batch {i + 1} of {num_batches}...")
                write_images_to_pdf(batch_files, output_pdf_path if i == 0 else f"temp_{i}.pdf")

            # Merge batch PDFs into a single file
            if num_batches > 1:
                from PyPDF2 import PdfMerger
                merger = PdfMerger()
                merger.append("output.pdf")
                for i in range(1, num_batches):
                    merger.append(f"temp_{i}.pdf")
